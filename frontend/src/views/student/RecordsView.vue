<template>
  <div class="page records-page">
    ...
    <section class="record-summary" aria-label="识别记录摘要">
      <MetricCard label="记录总数" :value="records.length" unit="条" note="含图像识别与视频分析" tone="info" icon="files" />
      <MetricCard label="主导情绪" :value="dominantEmotionLabel" note="按当前记录统计" tone="neutral" icon="pie" />
      <MetricCard label="负向占比" :value="`${negativeRatio}%`" note="悲伤、愤怒、恐惧、厌恶" :tone="negativeRatio >= 30 ? 'warning' : 'info'" icon="trend" />
      <MetricCard label="高风险记录" :value="highRiskCount" unit="条" note="建议结合详情复核" :tone="highRiskCount ? 'danger' : 'stable'" icon="warning" />
    </section>

    <RecordInsightCharts v-if="records.length > 0" :records="records" />
    <RecognitionRecordTable :records="records" :loading="loading" :focus-record-id="route.query.record" />
  </div>
</template>
